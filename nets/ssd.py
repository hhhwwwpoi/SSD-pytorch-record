...
class SSD(nn.Module):
    def __init__(self, phase, base, extras, head, num_classes, confidence, nms_iou, backbone_name):  #phase, backbone, extra_layers, (loc_layers, conf_layers), num_classes, confidence, nms_iou, backbone_name
        ...
        with torch.no_grad():
            self.priors     = torch.tensor(self.priorbox.forward()).type(torch.FloatTensor)  #24656，4
    ...
    def forward(self, x):
        ...
        for (x, l, c) in zip(sources, self.loc, self.conf):
            loc.append(l(x).permute(0, 2, 3, 1).contiguous())         # 4,64,64,4*4
            conf.append(c(x).permute(0, 2, 3, 1).contiguous())        # 4,64,64,4*21
        ...
        if self.phase == "test":
            ...
            output = (
                loc.view(loc.size(0), -1, 4),                   # 4, 64*64*4+…… ,4
                conf.view(conf.size(0), -1, self.num_classes),  # 4, 64*64*4+…… ,21
                self.priors                                     # 24656,4  所有先验框
            )
        return output
...
def get_ssd(phase, num_classes, backbone_name, confidence=0.5, nms_iou=0.45):
    ...
    #---------------------------------------------------#
    if backbone_name=='vgg':
        backbone, extra_layers = add_vgg(3), add_extras(1024, backbone_name)  # n,1024,32,32   n,256,4,4
        mbox = [4, 6, 6, 6, 4, 4]
    else:
    ...
    #
    #   (64,64,512), (32,32,1024), (16,16,512), 
    #   (8,8,256), (6,6,256), (4,4,256)
    #-------------------------------------------------------------#
    SSD_MODEL = SSD(phase, backbone, extra_layers, (loc_layers, conf_layers), num_classes, confidence, nms_iou, backbone_name)  #confidence=0.5, nms_iou=0.45
    return SSD_MODEL
