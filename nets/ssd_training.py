...
class MultiBoxLoss(nn.Module):
    def __init__(self, num_classes, overlap_thresh, prior_for_matching,          #   Config['num_classes'], 0.5, True,
                 bkg_label, neg_mining, neg_pos, neg_overlap, encode_target,     #   0, True, 3, 0.5, False,
                 use_gpu=True, negatives_for_hard=100.0):                        #   Cuda
    ...
    def forward(self, predictions, targets):
        ...
        #
        #   batch_conf.gather(1, conf_t.view(-1, 1))是找到每个先眼眶(对应label)的损失值    dim = 98624,1
        # 
        #--------------------------------------------------#
        batch_conf = conf_data.view(-1, self.num_classes)
        # 这个地方是在寻找难分类的先验框 loss_c = -log(softmax()) = logsumexp - xj 
        # 这里只取了每个先眼眶对应label的那个置信度
        ...
        _, idx_rank = loss_idx.sort(1)    #idx_rank为原始数据(未逆序排序)下的值的排名
        #--------------------------------------------------#
        #   求和得到每一个图片内部有多少正样本
        #   num_pos     (num, )
        #   neg         (num, num_priors)
        ...
        neg = idx_rank < num_neg.expand_as(idx_rank)  #按排名大小取出负样本
